...
def get_free_times(begin_time, end_time, begin_date, end_date, event_list):
    ...
    if(time2 >= time1):
        return [].append([].append([].append({'begin': arrow.now().isoformat(), 'end': arrow.now().isoformat(), 'name': "invalid time range"})))
    if(date2 > date1):
        return [].append([].append([].append({'begin': arrow.now().isoformat(), 'end': arrow.now().isoformat(), 'name': "invalid date range"})))
